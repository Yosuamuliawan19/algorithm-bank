...
void solve(int t){
     // try to add the edges one by one
     for (auto cur: graph){
          // if graph turns out to be in the same set,
          if (sameSet(cur.x, cur.y)){
               // cout << cur.x +1<< " and " << cur.y+1 << " are in the same set !!!\n";
               ll sp = dijkstra(cur.x, cur.y);
               // cout << "Demand for " << cur.w << ", already have " << sp << endl;
          ...
          // unify in UFDS, and link the adjacency list
          unify(cur.x, cur.y);
          link(cur.x, cur.y, cur.w);
     }
     // to make the description with no repetition
